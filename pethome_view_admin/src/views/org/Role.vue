    ...
    <el-dialog :title="title" :visible.sync="saveFormVisible" :close-on-click-modal="false">
      ...
      <el-form :model="saveForm" label-width="80px" :rules="saveFormRules" ref="saveForm">
        ...
        <el-form-item label="分配菜单" prop="menus">
          <el-cascader v-model="saveForm.menuIds" clearable
                       placeholder="试试搜索：指南"
                       :options="menuTree"
                       :props="{
									multiple: true,
        ...
        <el-form-item label="分配权限" prop="permissions">
          <el-select v-model="saveForm.permissionIds" multiple placeholder="请选择">
            <el-option
...
export default {
  ...
  methods: {
    ...
    handleEdit: function (index, row) {
      ...
      //处理回显的菜单
      let roleMenuIds = this.saveForm.menuIds;
      let roleMenuFullIds = [];
      for (let rmi of roleMenuIds) {
        let path = this.menuPath[rmi].substring(1);
        let idPath = path.split("/");
        roleMenuFullIds.push(idPath);
      }
      this.saveForm.menuIds = roleMenuFullIds;
      //弹出对话框
    ...
    //获取菜单路径，用来处理菜单的数据回显
    getMenuPath() {
      this.$http.get("/menu/menuPath").then(res => {
        if (res.data) {
          this.menuPath = res.data;
        }
      });
    }
  },
